 ...
 * UPDATED: Raw vitals sync integration (realtime + polling)
 */
...
async function initDashboard() {
    ...
    // Initialize raw_vitals sync (realtime + polling)
    if (typeof initRawVitalsSync === 'function') {
        await initRawVitalsSync();
    } else {
        console.warn('initRawVitalsSync not available - raw vitals sync disabled');
    }
...
function setupEventListeners() {
    ...
    window.addEventListener('beforeunload', () => {
        ...
        if (typeof cleanupRawVitalsSync === 'function') {
            cleanupRawVitalsSync();
        }
    });
}
...
async function refreshData(type) {
    ...
    if (type === 'patients') {
        // Also trigger raw vitals sync on manual refresh
        if (typeof syncRawVitals === 'function') {
            await syncRawVitals();
        }
